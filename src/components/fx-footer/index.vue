  ...
  <div class="relative z-30 footer">
    <van-tabbar route v-model="active" active-color="#1194F7" @change="changeIndex" fixed safe-area-inset-bottom>
      <van-tabbar-item name="optional" to="/optional">
        ...
        <template #icon="props">
          <img :src="active == 'optional' ? icon.optional.active : icon.optional.inactive" />
        </template>
      </van-tabbar-item>
      <van-tabbar-item name="quotes" to="/quotes/index?tabActive=0">
        <span :class="[active === 'quotes' ? 'active' : '']">{{ $t("quotes") }}</span>
        <template #icon>
          <img :src="active == 'quotes' ? icon.quotes.active : icon.quotes.inactive" />
        </template>
      </van-tabbar-item>
      ...
      <van-tabbar-item name="news" to="/news">
        ...
        <template #icon>
          <img :src="active == 'news' ? icon.news.active : icon.news.inactive" />
        </template>
      </van-tabbar-item>
      ...
      <van-tabbar-item name="mine" to="/my">
        ...
        <template #icon>
          <img :src="active == 'mine' ? icon.mine.active : icon.mine.inactive" />
        </template>
      </van-tabbar-item>
    </van-tabbar>
  </div>
