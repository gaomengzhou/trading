<template>
  <div class="list-quatation">
    ...
    <van-list>
      <van-cell class="tab">
        <div class="flex items-center w-full text-grey font-12">
            ...
            <ul class="flex justify-evenly w-full items-center" @click="onItemClick(item)">
              <li class="flex items-center mid">
                <p class="flex flex-col items-start">
                  <span class="font-600 font-14 text-left text-color name"> {{ item.name || '--' }} </span>
                  <span class="flex font-12 flex items-center text-left leading-0.2 fullname">{{ item.symbolFullName ||
                    '--' }}</span>
                </p>
              </li>
              <li class="flex flex-col mid">
                <p class="w-153 font-31 h-71 text-color bg-green border-0 text-center" v-if="item.changeRatio > 0">
                  +{{ item.changeRatio }}%</p>
                <p class="w-153 font-31 h-71 text-color bg-red border-0 text-center btn" v-else>
