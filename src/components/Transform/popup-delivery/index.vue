<template>
  <div id="cryptos">
    <div class="popup-delivery w-700 font-28 rounded-2xl overflow-hidden">
      ...
      <div v-if="showBtns" class="flex justify-between px-38 pb-61">
        <p :class="{ 'btnMain': detailData.state !== 'created', 'bgDark': detailData.state === 'created' }"
          class="w-255 h-90 rounded-lg flex justify-center items-center mr-20 text_color" @click="onClose">{{ $t('关闭') }}</p>
        <p @click="continueToBuy"
          :class="{ 'btnMain': detailData.state === 'created', 'bgDark': detailData.state !== 'created', 'disableBtn': disabled }"
          class="w-385 h-90 rounded-lg flex justify-center items-center text_color">{{ $t('继续下单') }}</p>
      </div>
    </div>
  </div>
</template>
