...
export default {
  ...
  watch: {
    deepBuy: {
      handler(val) {
        if (val.length > 0 && this.deepSell.length > 0 && this.activatedFlag) {
          let arry1 = [];
        ...
        series: [
          {
            ...
            label: {
              ...
              padding: 10,
              fontSize: 12,
              color: _this.THEME == 'white' ? "#666" : "#fff",
              backgroundColor: _this.THEME == 'white' ? "rgba(0, 0, 0, .3)" : "rgba(255, 255, 255, .3)",
              formatter: function (params) {
                return [
                  `${_this.$t('价格')} ：{a|${params.data[0]}}`,
                  `${_this.$t('数量')} ：{a|${params.data[1]}}`,
                  `${_this.$t('成交额')} ：{a|${(params.data[1] * params.data[0]).toFixed(5)}}`,
                ].join("\n");
              },
              rich: {
                a: {
