    ...
    <div class="loanHistory">
      ...
      <div class="tabMenu flex justify-start items-center">
        <div class="tabList">
          <div
            class="tab"
            :class="item.id === activeTabIndex ? 'activeTab' : ''"
      ...
      <div class="container px-32">
        ...
        <div v-if="loanOrderList && loanOrderList.length > 0">
          <div
            class="historyData flex justify-between py-40 border-b-color textColor"
          ...
          >
            <div class="left">
              ...
              <div class="mb-30">
                <span>{{ $t("借款金额") }}: </span>
                <span :class="item.status === 5 ? 'text-red' : 'text-green'"
                  >{{ item.status === 5 ? "-" : "+" + item.quota }}
                  {{ item.symbol }}</span
                >
              </div>
            ...
            <div class="right text-center text-white">
              <div
                class="loanBtn mb-20"
                @click="$router.push('/customerService')"
                v-if="item.state === 2"
              >
                {{ $t("申请还款") }}
              </div>
              <div
                class="loanBtn mb-20"
                :style="{ background: loanBgText(item.state) }"
                v-else
              >
                {{ loanText(item.state) }}
              </div>
              <div
                class="loanBtn"
                style="background: #00a7ee"
                @click="getLoanDetail(item.uuid)"
              >
                {{ $t("详情") }}
              </div>
            </div>
        ...
        <div class="px-32 py-40">
          ...
          <div class="loanList">
            ...
            <div class="flex justify-between py-30 border-b-color">
              ...
              <div class="flex align-center">
                <span class="font-600" :style="{ color: loanBgText(loanData.state) }">{{
                  loanText(loanData.state)
                }}</span>
              </div>
            </div>
            <div
              class="flex justify-between py-30 border-b-color"
              v-if="loanData.state === 2"
            >
              <div>
