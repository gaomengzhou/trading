        ...
        <div class="pt-30 px-32 pb-32 tabBackground">
          <template v-if="detail.direction == 'buy'">
            <template v-if="detail.state == 0">
              <div class="font-40 c2cColor flex items-center" v-if="time > 0">{{ $t('订单将在倒计时结束时取消。') }}&nbsp;<span
                  class="text-blue">
                  <!-- {{ time
                |
                format }} -->
                  <van-count-down class="flex font-700 mx-10 textColor" :time="time">
          ...
          <div class="flex justify-between items-center mt-31">
            ...
            <div class="font-30 text-blue" @click="$router.push('/cryptos/appeal')">{{ $t('举报') }}</div>
          </div>
