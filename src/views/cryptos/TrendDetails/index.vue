  ...
  <div id="cryptos"> 
    <div class="trend-detail pb-90" :class="isNight ? 'bg-night' : 'bg-white'">
      ...
      <div :class="{ slide2: animated1, slide1: animated2 }">
        <div class="data-index">
          <div class="flex flex-1 px-32 mt-40">
            <div class="flex flex-col font-22">
              ...
              <p class="text-grey mt-10">{{ $t('标记价格') }} {{ quote.open || 0 }}</p>
            </div>
            <div class="flex-wrap flex-1 justify-end font-22">
              <div class="flex justify-end">
                <div class="flex w-1/2 flex-col justify-between w-177">
                  <p class="text-grey">{{ $t('24h最高价') }}</p>
                  <p>{{ quote.high || 0 }}</p>
                </div>
                <div class="flex w-1/2 flex-col justify-between w-177 buy-title">
        ...
        <p class="status-info" v-if="quote.market && quote.market.status">
        ...
        <div class="w-full h-20 " :class="isNight ? 'bg-black' : 'white'"></div>
        <!-- tab 区域-->
              ...
              <div class="flex px-32 justify-between px-32">
                ...
                <ul class="flex-1 flex flex-col ml-20">
                  <li class="font-28 text-grey mt-30 mb-15 w-full">
                    <p>{{ $t('卖出') }}</p>
                    <p class="flex  justify-between  font-22">
                      <span class="ml-5">{{ $t('价格') }}{{ queryType == 'cryptos' ? '(USDT)' : '(USD)' }}</span>
                      <span class="buy-title">{{ $t('数量') }}({{ symbolData.toUpperCase() }})</span>
                    </p>
                  </li>
        ...
        <div
          ...
          <div ref="toBuy" class="w-270 h-88 bg-green flex justify-center items-center rounded-lg font-28"
            @click=" onSubmit('buy')">
            {{ tab == 1 ? $t('开多') : $t('买多') }}
          </div>
          <div ref="toSell" class="w-270 h-88 bg-red flex justify-center items-center rounded-lg font-28"
            @click="onSubmit('sell')">
            {{ tab == 1 ? $t('开空') : $t('买空') }}</div>
        </div>
      </div>
      <van-popup v-model:show="show" class="rounded-2xl" :key="initFutrue.session_token">
        <popup-confirm-order :symbolName="symbolName" :paras="initFutrue.para" :symbol="symbol" :direction="direction" :balance="initFutrue.amount"
