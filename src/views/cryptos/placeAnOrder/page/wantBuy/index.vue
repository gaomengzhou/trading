<template>
  <div id="cryptos">
    <div id="wantBuy" class="c2cBackground fixed left-0 right-0 top-0 bottom-0 wantBuy">
      <div class="w-full h-122 pl-40 pr-31 box-border c2cBackground flex items-center c2cHeader">
        <van-icon name="arrow-left" class="font-40" @click="goback" />
        ...
        <div class="w-232 h-64 px-23 box-border box-radius text-black flex justify-around items-center"
          ...
          <span class="mx-28 w-2 h-50 ">|</span>
                ...
                <div class="flex">
                  <div class="flex-1 textColor">
                    <div class="flex items-center">
                      ...
                      <img class="w-30 h-30" src="@/assets/image/otc/buy/star.png" alt="">
                    </div>
                    <div class="font-28 text-grey mt-31">{{ $t('单价') }}</div>
                    <div class="mt-10">
                      <span class="font-28 mr-10">{{ item.currency }}</span>
                      <span class="font-40">{{ (item.symbol_value / 1).toFixed(2) }}</span>
                    </div>
                    <div class="mt-26">
                      <span class="font-28 text-grey mr-10">{{ $t('数量') }}</span>
                      <span class="font-28">{{ item.coin_amount && (item.coin_amount /
                        1).toFixed(item.symbol.toUpperCase() === 'USDT' ? 2
                          :
                          4) }}</span>
                    </div>
                    <div class="mt-13">
                      <span class="font-28 text-grey mr-10">{{ $t('限额') }}</span>
                      <span class="font-28 ">{{ currencySymbol }} {{ (item.investment_min / 1).toFixed(2) }} - {{
                        currencySymbol }} {{ (item.investment_max / 1).toFixed(2) }}</span>
                    </div>
                    <div class="flex mt-36 font-30">
