...
const onRouteTwo = (item) => {
  ...
  if (symbolType.value === 'TW-stocks') {
    ...
    router.push(`/quotes/detail?symbol=${item.symbol}&type=${symbolType.value}`);
  }
  fetchQuotes();
  getItemSummary(symbol.value);
  getConstituentStockList(symbol.value);
...
const handleGoBack = () => {
  if (route.query.from === "trade") {
    ...
    router.push(`/quotes/hotModules`);
  } else if (route.query.isOptional == 1) {
    router.push(`/optional/index`);
  } else if (route.query.symbolType == 'TW-stocks') {
    router.push(`/quotes/index?tabActive=5`);
  }
  // else if (route.query.isOptional == 2){
  ...
  if (path === "/quotes/openTrade") {
    router.push({
      ...
      query: {
        tabActive,
        symbol: symbol.value,
        type: symbolType.value == 'TW-stocks' ? "TW-stocks" : "indices",
      },
    });
