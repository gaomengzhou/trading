  ...
  <div class="pb-fix">
    <div class="container-box">
      <div class="search-container flex items-center">
        <van-search class="search-input" v-model="searchVal" @input="onClickButton" :placeholder="$t('searchKeys')"
            ...
            <li class="flex" v-for="(item, index) in searchAllList" @click.stop="itemClick(item)">
              <div class="flex-l">
                <div class="flex-l-item logo">
                  <img :src="`${HOST_URL}/symbol/${item.symbolData ? item.symbolData : item.symbol_data}.png`" alt="">
                </div>
              ...
              <div class="flex-r">
                <div class="flex-r-item data-box">
                  <div class="data-item">
                    <span v-if="item.isCollect">{{ t('alreadyMyfavorites') }}</span>
        ...
        <div class="symbol-table">
          <ul>
            <li class="flex" v-for="(item, index) in historyAllList" @click.stop="itemClick(item)">
              <div class="flex-l">
                <div class="flex-l-item logo">
                  <img :src="`${HOST_URL}/symbol/${item.symbol_data ? item.symbol_data : item.symbolData}.png`" alt="">
                </div>
                <div class="flex-l-item">
                  <p class="name-short">{{ item.symbol.toUpperCase() }}</p>
                  <p class="name">{{ item.name }}</p>
...
const itemClick = (data) => {
  ...
  if (data.type == "cryptos") { //跳转需你币
    ...
    router.push('/quotes/detail?symbol=' + data.symbol + '&isOptional=2')
...
const getIsItemHasAddGlobal = () => {
  selectInfo.value.isCollect = true
}
//打开自选弹窗
