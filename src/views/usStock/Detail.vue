  ...
  <section class="pb-40">
    <div class="container-box">
      ...
      <section class="kline-container flex">
        ...
        <div class="order-book-container" v-if="timeLabelActive === 0 && symbolType !== 'HK-stocks'">
          <!-- <keep-alive>
            <trade-deep-data :symbol="symbol" v-if="symbol" :price="price" class="trade-deep-container" />
          </keep-alive> -->
          <ul>
            <li class="flex font-10 orderbook sell" v-for="(item, index) in tradeList" :key="index">
              <span class="text">{{ item.timestamp ? dayjs(item.timestamp * 1000).format('HH:mm') : '--'
              }}</span>
              <span>&nbsp;&nbsp;</span>
              <div class="justify-between flex flex-1">
                <span :class="item.trade_volume >= 100 ? 'text-green' : 'text-red'">{{ item.current ?
                  Number(item.current).toFixed(3) : '--' }}</span>
                <span class="text-right textColor" :class="item.trade_volume >= 100 ? 'text-green' : ''">{{
                  formatMoney(item?.trade_volume) }}</span>
              </div>
            </li>
          </ul>
        </div>
    ...
    <van-popup class="popup" round v-model:show="show" close-icon-position="top-left" position="left" @closed="onClose">
      ...
      <div class="pl-10 pr-10">
        <div class="flex justify-between mb-10" v-for="item in iconList" :key="item.name" @click="onRouteTwo(item)">
          <div>
            <div class="font-700 textColor popup-title">{{ item.name || '--' }}</div>
...
import deepChart from '@/components/Transform/deepChart/index2.vue'
...
const handleGoBack = () => {
  ...
  if (route.query.isMore == 1) {
    router.push(`/quotes/UsStockMore?index=${route.query.tabIndex}&typName=${route.query.typName}`)
    return
  }
  ...
  if (fromIndexDetail) {
    ...
    let backIndex = route.query.symbolType === 'US-stocks' ? '3' : route.query.symbolType === 'HK-stocks' ? '4' : '5'
    onRoute(`/quotes/index?tabActive=${backIndex}`)
  }
}
